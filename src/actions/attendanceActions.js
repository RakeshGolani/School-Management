...
export async function getAttendanceAction({ date, entity_type = 'STUDENT', class_name = '', section = '' } = {}) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/school/attendance?${query.toString()}`, {
      method: 'GET',
...
export async function saveBulkAttendanceAction({ date, entity_type = 'STUDENT', class_name = '', section = '', academic_year_id = null, records = [] }) {
  try {
    const response = await fetch(`${BACKEND_URL}/school/attendance/bulk`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        date,
...
export async function getAttendanceSummaryAction(date = '', academic_year_id = null) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/school/attendance/summary${queryString}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
      cache: 'no-store'
    });
