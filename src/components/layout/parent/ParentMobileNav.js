...
export default function ParentMobileNav({ user, navItems, onOpenDrawer }) {
  const pathname = usePathname();

  const mainNavItems = (navItems && navItems.length > 0)
    ? navItems.slice(0, 4)
    : [
        { label: 'Hub', href: '/parent/dashboard', icon: LayoutDashboard },
        { label: 'Live Bus', href: '/parent/bus-tracking', icon: Bus },
