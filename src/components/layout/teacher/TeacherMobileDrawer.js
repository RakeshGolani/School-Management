...
export default function TeacherMobileDrawer({
  isOpen,
  onClose,
  navItems,
  teacherName,
  ...
  const quickLinks = (navItems && navItems.length > 0)
    ? [
        ...navItems.map(item => ({
          ...
          href: item.href,
          icon: item.icon,
          desc: item.desc || (item.href === '/teacher/leaves' ? 'Review & approve student leaves' : item.label)
        })),
        { label: 'My Profile & ID Card', href: '/teacher/profile', icon: GraduationCap, desc: 'Faculty credentials & ID' }
      ]
    : [
        { label: 'Dashboard', href: '/teacher/dashboard', icon: LayoutDashboard, desc: 'Overview & daily metrics' },
        { label: 'Mark Attendance', href: '/teacher/attendance', icon: CheckCircle2, desc: 'Roll call for assigned class' },
        { label: 'Teaching Schedule', href: '/teacher/timetable', icon: CalendarDays, desc: 'Allocated periods & rooms' },
        { label: 'Class Students', href: '/teacher/students', icon: Users, desc: 'Directory & contact info' },
        { label: 'Leave Approvals', href: '/teacher/leaves', icon: FileText, desc: 'Review & approve student leave requests' },
        { label: 'My Profile & ID Card', href: '/teacher/profile', icon: GraduationCap, desc: 'Faculty credentials & ID' },
      ];
      ...
      <div className="relative z-10 w-full max-h-[88vh] bg-white rounded-t-[2rem] border-t border-slate-200/90 shadow-2xl flex flex-col overflow-hidden animate-slideUp">
        ...
        <div className="px-6 py-2 flex items-center justify-between border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-black text-slate-900">Faculty Menu & Shortcuts</span>
          </div>
          <button
            onClick={onClose}
          ...
          >
            <div className="flex items-center gap-3.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-black text-slate-900 truncate group-hover:text-primary-600 transition-colors">{teacherName}</p>
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 shrink-0">Active</span>
                </div>
                <p className="text-xs font-mono text-primary-600 font-bold">{employeeId}</p>
                <p className="text-[11px] text-slate-500 truncate">{classTeacherFor}</p>
              </div>
            </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between gap-3 text-xs">
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-[10px] font-bold text-primary-600">Faculty Desk</span>
              </div>
            </div>
          </div>
