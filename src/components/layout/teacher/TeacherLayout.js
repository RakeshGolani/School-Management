...
export default function TeacherLayout({ user, children }) {
  ...
  const teacherPhoto = user?.image_url || user?.photo;
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 text-slate-900 font-sans selection:bg-primary-500 selection:text-white">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        {/* MODULAR TEACHER FOOTER (Hidden on mobile to avoid bottom overlap) */}
        <div className="hidden lg:block">
          <TeacherFooter schoolName={schoolName} />
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        title="Sign Out Faculty Desk"
        message={`Are you sure you want to sign out from ${teacherName}'s faculty session?`}
