...
export default function StudentUserDropdown({
  ...
  const initialChar = (studentName || 'S').trim().charAt(0).toUpperCase();
      ...
      >
        ...
        <div className="relative shrink-0 group-hover:scale-105 transition-transform">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-primary-50 border border-primary-500/25 flex items-center justify-center text-primary-700 font-black text-xs overflow-hidden relative shadow-2xs">
            {studentPhoto ? (
              <img src={studentPhoto} alt={studentName} className="absolute inset-0 w-full h-full object-cover rounded-[inherit] z-10" />
            ) : (
        ...
        <div className="hidden sm:block text-left pr-1">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="text-[10px] font-mono font-bold text-primary-600">{admissionNumber}</span>
        ...
        <div className="absolute right-0 mt-2 w-72 bg-white p-3.5 rounded-3xl space-y-3 shadow-2xl z-[100] animate-fadeIn border border-slate-100">
          ...
          <div className="flex items-center space-x-3 border-b border-slate-100 pb-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] font-mono text-primary-600 font-bold truncate">{admissionNumber}</p>
              {className && (
                <p className="text-[10px] text-slate-500 truncate mt-0.5">{className}</p>
              )}
            </div>
          </div>
