...
export default function StudentIdCardModal({ isOpen, onClose, student, schoolInfo }) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `student_id_card_${fullName.replace(/\s+/g, '_')}.pdf`;
  ...
  const modalContent = (
    <div 
      ...
      onClick={onClose}
    >
      {/* CSS Print Styles for Exact ID Card Printing */}
      <style jsx global>{`
        @media print {
          html, body {
            margin: 0 !important;
          ...
          #modal-id-card-printable, #modal-id-card-printable * {
            visibility: visible !important;
          }
          #modal-id-card-printable {
          ...
          #modal-id-card-printable .w-\\[320px\\] {
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Official printable identity badge</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <StudentIdCard student={student} schoolInfo={schoolInfo} activeYear={activeYear} containerId="modal-id-card-printable" />
