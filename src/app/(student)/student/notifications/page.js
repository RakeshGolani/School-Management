...
import StudentNotificationSkeleton from '@/components/skeletons/student/StudentNotificationSkeleton';
...
export default function StudentNotificationsPage() {
  ...
  const handleMarkAsRead = async (id, actionUrl) => {
    try {
      const res = await markStudentNotificationReadAction(id);
      if (res && res.success) {
  ...
  const handleMarkAllRead = async () => {
    try {
      const res = await markAllStudentNotificationsReadAction();
      if (res && res.success) {
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">
              Student Activity & Notification Center
            </h1>
          </div>
          <p className="text-xs text-slate-500 max-w-xl">
            Live updates on attendance roll calls, leave status, class announcements, and bus transit telemetry.
          </p>
        </div>
