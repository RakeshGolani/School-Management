...
export default function ParentBusTrackingClient({ initialData }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto animate-fadeIn pb-24 sm:pb-8">
      ...
      {/* If Ward Does Not Have Bus Transit Subscribed */}
      {!isEnabled && (
        <div className="p-6 rounded-3xl bg-amber-50/80 border border-amber-200 text-amber-950 space-y-3">
          <div className="flex items-center gap-2 font-black text-amber-900">
            <AlertCircle size={20} className="text-amber-600" />
            <span>Smart Bus Transit Not Subscribed for {wardDisplayName}</span>
          </div>
          <p className="text-xs text-amber-800 leading-relaxed max-w-2xl">
            {wardDisplayName} is not currently enrolled in the institutional Smart Bus transit fleet service. If you would like to enable morning pickup and afternoon drop services, please contact your school administrator.
          </p>
          <div className="pt-2 flex items-center gap-4 text-xs font-bold text-amber-900">
            <span>School Desk: {school?.phone || '079-2658-9900'}</span>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-2.5">
              ...
              <div className="flex items-center justify-between text-[11px] font-mono font-bold pt-1 border-t border-slate-100 text-emerald-700">
                ...
                <span className="text-amber-700">Drop: {assignedStop?.drop_off_time || '--'}</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-black text-slate-900">Assigned Bus</h4>
              </div>
              <div className="font-mono font-black text-xs text-slate-900">
                {bus?.bus_number || 'Smart Bus Fleet'}
              </div>
              <div className="text-[11px] text-slate-500 font-medium pt-1 border-t border-slate-100 truncate">
                Driver: {bus?.driver_name || 'Ramesh Patel'}
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-black text-slate-900">NFC Pass UID</h4>
              </div>
              <div className="font-mono font-black text-xs text-primary-700 truncate">
                {studentInfo?.nfc_card_uid || 'STUDENT_CARD_001'}
              </div>
              <div className="text-[11px] text-slate-500 font-medium pt-1 border-t border-slate-100 flex items-center gap-1">
        ...
        {/* Right Column: Route Stop Sequence Timeline */}
        <div className="lg:col-span-4 p-5 sm:p-6 rounded-3xl bg-white border border-slate-200 shadow-2xs space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800 flex items-center gap-2">
              <Compass size={15} className="text-primary-600" /> Route Stop Sequence
            </h3>
          ...
          <div className="space-y-3 relative before:absolute before:inset-y-3 before:left-3.5 before:w-0.5 before:bg-slate-200 max-h-[480px] overflow-y-auto pr-1">
            {allStops.map((s, idx) => {
              const isMyStop = Boolean(s.is_my_stop || (assignedStop && assignedStop.id === s.id));

              return (
                <div key={s.id || idx} className="flex items-start space-x-3 relative z-10 text-xs">
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-[10px] shrink-0 ${
                      isMyStop
                        ? 'bg-amber-500 text-slate-950 ring-4 ring-amber-500/20 animate-pulse font-black shadow-md'
                        : 'bg-white text-primary-700 border-2 border-primary-400'
                    }`}
                  >
                    #{s.sequence || (idx + 1)}
                  </div>
                  <div className="min-w-0 flex-1 pt-0.5">
                    <div className="flex items-center justify-between gap-1">
                      <span className={`font-bold truncate ${isMyStop ? 'text-amber-900 font-black' : 'text-slate-900'}`}>
                        {s.stop_name}
                      </span>
                    ...
                    {isMyStop && (
                      <span className="inline-block mt-0.5 text-[9px] font-black uppercase tracking-wider text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200">
