...
import TeacherNotificationSkeleton from '@/components/skeletons/teacher/TeacherNotificationSkeleton';
...
export default function TeacherNotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getTeacherNotificationsAction({
        page: currentPage,
        limit: 15,
        type: activeType,
        is_read: readFilter,
  ...
  const handleMarkAsRead = async (id, actionUrl) => {
    try {
      const res = await markTeacherNotificationReadAction(id);
      if (res && res.success) {
  ...
  const handleMarkAllRead = async () => {
    try {
      const res = await markAllTeacherNotificationsReadAction();
      if (res && res.success) {
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-50 via-white to-primary-50/40 border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 max-w-xl">
            Live updates on student leave requests, daily schedule adjustments, and institutional announcements.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto w-full lg:w-auto pb-1 lg:pb-0 scrollbar-none">
            {[
              ...
              { id: 'TIMETABLE', label: '⏰ Timetable & Proxy' },
