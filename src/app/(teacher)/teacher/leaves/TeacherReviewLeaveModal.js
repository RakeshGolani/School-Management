...
export default function TeacherReviewLeaveModal({
  isOpen,
  leave,
  onClose,
  onSubmit,
  ...
  const initial = (leave.student_name || 'S').charAt(0).toUpperCase();
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                Class Teacher Decision & Remarks
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/70 flex items-center justify-between gap-3">
            <div className="flex items-center space-x-3 min-w-0">
              <div className="w-11 h-11 rounded-2xl bg-primary-50 border border-primary-500/25 flex items-center justify-center text-primary-700 font-black text-sm flex items-center justify-center shrink-0 overflow-hidden relative shadow-2xs">
                <span>{initial}</span>
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 text-[11px] text-slate-500 font-medium mt-0.5 flex-wrap">
                  ...
                  <span>•</span>
                  <span>Class: {leave.class_name}</span>
                  {leave.roll_number && (
                    <>
                      <span>•</span>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/60 text-slate-800 space-y-1">
              ...
              {leave.emergency_contact && leave.emergency_contact !== '--' && (
                <div className="pt-1.5 mt-1 border-t border-slate-200/60 flex items-center gap-1.5 text-[11px] text-slate-500 font-mono">
                  ...
                  <span>Emergency Contact: <strong className="text-slate-800">{leave.emergency_contact}</strong></span>
                </div>
              )}
            </div>
          </div>

          {/* Review Decision Selector (Approve / Reject) */}
          <div className="space-y-2">
            <label className="block text-xs font-black text-slate-800 uppercase tracking-wider">
              Class Teacher Decision <span className="text-rose-500">*</span>
            </label>
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-black text-slate-800 uppercase tracking-wider">
              Feedback Remarks / Notes (Optional)
            </label>
            <textarea
              rows={2}
              value={remarks}
              onChange={(e) => setRemarks(e.target.value)}
              placeholder="Add advice or remarks for student (e.g. Approved. Take rest and catch up on homework.)..."
